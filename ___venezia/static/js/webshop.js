...
async function showFlavorSelectionModal(productId, maxFlavors, productName) {
    ...
    try {
        ...
        // Sugeridos por disponibilidad
        const withStock = flavors.filter(f => typeof f.stock === 'number');
        if (withStock.length) {
            const suggested = withStock
                .filter(f => f.stock > 0.5)
                .sort((a,b) => (b.stock||0) - (a.stock||0))
                .slice(0, 5);
            if (suggested.length) {
                const suggWrap = document.createElement('div');
                suggWrap.className = 'mb-3';
                ...
                suggested.forEach(s => {
                    ...
                    b.textContent = `${s.name} (${s.stock.toFixed(2)} kg)`;
                    b.onclick = () => {
                        const cb = document.getElementById(`flavor${s.id}`);
                        if (cb) {
                ...
                auto.onclick = () => {
                    const current = Array.from(document.querySelectorAll('.flavor-checkbox:checked')).length;
                    const toAdd = Math.max(0, maxFlavors - current);
                    let added = 0;
                ...
                suggWrap.appendChild(btns);
                flavorsListDiv.appendChild(suggWrap);
                const divider = document.createElement('hr');
                flavorsListDiv.appendChild(divider);
            }
        }

        // Preferencias recientes: preseleccionar si hay cupo
        if (recent.length) {
            let remaining = maxFlavors;
            // marcar los ya seleccionados
            document.querySelectorAll('.flavor-checkbox:checked').forEach(()=>remaining--);
            for (const rid of recent) {
                if (remaining <= 0) break;
                const cb = document.getElementById(`flavor${rid}`);
                if (cb && !cb.checked) {
                    cb.checked = true;
                    cb.dispatchEvent(new Event('change'));
                    remaining--;
        ...
        flavors.forEach(flavor => {
            ...
            div.innerHTML = `
                ...
                <label class="form-check-label" for="flavor${flavor.id}">
                    ${flavor.name}
                    ${typeof flavor.stock === 'number' ? `<span class="badge badge-${flavor.stock <= 0.5 ? 'danger' : (flavor.stock <= 1 ? 'warning' : 'secondary')} ml-2">${flavor.stock.toFixed(2)} kg</span>` : ''}
                </label>
            `;
