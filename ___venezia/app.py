...
from mercadopago_integration import MercadoPagoIntegration, mp_integration
...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('SQLALCHEMY_DATABASE_URI', 'sqlite:///venezia.db?timeout=30')
...
def assign_batch():
    if request.method == 'GET':
        ...
        productions = Production.query.filter(
            ...
            Production.assigned_store_id.is_(None)
        ).order_by(Production.production_date.desc()).all()
        return render_template('assign_batch.html', stores=stores, productions=productions)
...
def edit_store_stock(store_id):
    ...
    # Get products (incluir Presentaciones para envases)
    products = Product.query.join(
        ProductCategory, Product.category_id == ProductCategory.id
    ).filter(
        ProductCategory.name.in_(['Sabores', 'Postres Helados', 'Productos Envasados', 'Presentaciones']),
        Product.active == True
    ).all()
    ...
    # Productos de potes (para editar precios)
    pote_products = Product.query.filter(
        Product.active == True,
    ...
    return render_template('edit_stock.html',
                         ...
                         pote_products=pote_products,
                         show_store_selection=False)

@app.route('/store/<int:store_id>/update_pote_price', methods=['POST'])
def update_pote_price(store_id):
    try:
        product_id = request.form.get('product_id')
        new_price = request.form.get('new_price')
        if not product_id or not new_price:
            raise BadRequest('Parámetros inválidos')
        ...
        db.session.rollback()
        flash(f'Error al actualizar precio: {str(e)}', 'danger')
        return redirect(url_for('edit_store_stock', store_id=store_id))
...
def get_cart():
    try:
        ...
        for idx, item in enumerate(cart):
            ...
            cart_html += f'''
            <div class="cart-item mb-2" data-index="{idx}" tabindex="0">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="text-end">
                        <div class="mb-1">${float(item['total_price']):.2f}</div>
                        <div class="btn-group btn-group-sm" role="group" aria-label="Editar ítem">
                            <button class="btn btn-outline-secondary cart-dec" data-index="{idx}" aria-label="Restar 1">-</button>
                            <button class="btn btn-outline-secondary cart-inc" data-index="{idx}" aria-label="Sumar 1">+</button>
                            <button class="btn btn-outline-danger cart-del" data-index="{idx}" aria-label="Eliminar">×</button>
                        </div>
                    </div>
                </div>
            </div>
...
@app.route('/api/cart/update_item', methods=['POST'])
def cart_update_item():
    """Update cart item quantity by index with delta or direct quantity."""
    try:
        ...
        current_qty = int(cart[index].get('quantity', 1))
        if delta is not None:
            try:
                current_qty += int(delta)
            except Exception:
                return jsonify({'status': 'error', 'message': 'Delta inválido'}), 400
        ...
        if current_qty <= 0:
            ...
            cart[index]['quantity'] = current_qty
            cart[index]['total_price'] = float(cart[index]['unit_price']) * current_qty
        session['cart'] = cart
        session.modified = True
        # return updated cart
        return get_cart()
...
def process_sale():
    try:
        ...
        if mp.is_configured:
            try:
                ...
                if success:
                    ...
                    sale.mp_preference_id = response.get('id')
        ...
        # Prepare receipt payload before clearing cart
        receipt_items = []
        for item in cart:
            product = Product.query.get(item['product_id'])
            if not product:
...
@app.route('/api/payment_status/<int:sale_id>', methods=['GET'])
def api_payment_status(sale_id):
    try:
        sale = Sale.query.get_or_404(sale_id)
        # Fast path
        if sale.mp_status == 'approved' or sale.payment_status == 'approved':
            return jsonify({'status': 'approved'})
        if mp_integration.is_configured:
            # Prefer direct payment id if available
            if sale.mp_payment_id:
                success, status = mp_integration.get_payment_status(sale.mp_payment_id)
                if success:
                    sale.mp_status = status
                    sale.payment_status = status
                    sale.mp_last_updated = datetime.now()
                    db.session.commit()
                    return jsonify({'status': status})
            # Fallback to external_reference search (sale id)
            success, result = mp_integration.get_status_by_external_reference(str(sale.id))
            if success:
                sale.mp_payment_id = result.get('payment_id')
                sale.mp_status = result.get('status')
                sale.payment_status = result.get('status')
                sale.mp_last_updated = datetime.now()
                db.session.commit()
                return jsonify({'status': result.get('status')})
            return jsonify({'status': 'unknown'})
        return jsonify({'status': sale.mp_status or 'pending'})
    except Exception as e:
        ...
        if search:
            query = query.filter(
                or_(
                    ...
                    Provider.lastname.ilike(f'%{search}%'),
...
def mass_edit_minimum_stock(store_id):
    try:
        ...
        source_stock = Stock.query.filter_by(store_id=store_id, product_id=source_product_id).first()
        
        if not source_stock:
            return jsonify({'success': False, 'error': 'Producto fuente no encontrado'})
        
        # Update target products
        for target_id in target_product_ids:
            target_stock = Stock.query.filter_by(store_id=store_id, product_id=target_id).first()
...
def stock_data():
    ...
    for stock, product, store, min_stock in query.all():
        ...
        result.append({
            ...
            'product_id': product.id,
            'flavor': product.name,
            'current_quantity': float("%.2f" % (stock.quantity or 0)),
            'minimum_quantity': float("%.2f" % min_quantity),
            'status': 'Bajo stock' if (stock.quantity or 0) <= min_quantity else 'OK'
        })
        ...
        for order in orders:
            try:
                order_dict = {
                    ...
                    'status': order.current_status.name if order.current_status else None,
...
def update_production_order_status(order_id):
    try:
        order = ProductionOrder.query.get_or_404(order_id)
        data = request.get_json()
        
        if 'status_id' not in data:
            return jsonify({'error': 'Missing status_id field'}), 400
        ...
        # If changing from "terminado" (3) to another status, delete the production entry
        if old_status_id == 3 and new_status_id != 3 and order.production_id:
            production = Production.query.get(order.production_id)
            if production:
                # Only delete if it hasn't been assigned to a store yet
                if not production.assigned_store_id:
                    db.session.delete(production)
                order.production_id = None
        
        # If the new status is "terminado" (3), create a production entry
        elif new_status_id == 3 and not order.production_id:
            # Get the product details
            product = order.product
            
            # Generate batch number based on the product name
            batch_number = Production.generate_batch_number(product.name)
            ...
            production = Production(
                ...
                operator='Sistema',  # or get from session if you have user authentication
                notes=f'Creado automáticamente desde orden de producción #{order.order_number}',
                production_cost=0  # Set initial cost to 0
            )
            
            db.session.add(production)
            db.session.flush()  # This will assign the ID to the production object
            
            # Link the production to the order
...
@app.route('/update_order_progress', methods=['POST'])
def update_order_progress():
    try:
        ...
        if not order_id or progress is None:
        ...
        if progress < 0 or progress > order.quantity:
            return jsonify({'success': False, 'error': f'El progreso debe estar entre 0 y {order.quantity}'}), 400
        
        # Update the progress and status
        order.progress = progress
        if progress >= order.quantity:
            status = ProductionOrderStatus.query.filter_by(name='terminado').first()
            if status:
                order.status_id = status.id
        elif progress > 0:
            status = ProductionOrderStatus.query.filter_by(name='proceso').first()
            if status:
                order.status_id = status.id
        ...
        app.logger.error(f'Error updating order progress: {str(e)}')
        return jsonify({'success': False, 'error': 'Error interno del servidor'}), 500

@app.route('/api/production_orders/<int:order_id>/progress', methods=['PUT'])
def update_production_order_progress(order_id):
    try:
        order = ProductionOrder.query.get_or_404(order_id)
        data = request.get_json()
        progress = data.get('progress')
        ...
        return jsonify({'status': 'success', 'message': 'Progreso actualizado correctamente'})
