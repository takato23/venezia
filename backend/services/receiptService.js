...
class ReceiptService {
  ...
  generateReceipt(saleData) {
    ...
    if (saleData.customer_name) {
      doc.text(`Cliente: ${saleData.customer_name}`, 5, yPos);
      yPos += lineHeight;
    }
    
    if (saleData.customer_phone) {
      doc.text(`Tel: ${saleData.customer_phone}`, 5, yPos);
      yPos += lineHeight;
    }
    ...
    // In production, you could generate a QR code with the receipt ID
  ...
  generateInvoice(saleData) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(`Nombre: ${saleData.customer_name || 'Cliente General'}`, 25, 70);
    doc.text(`Teléfono: ${saleData.customer_phone || '-'}`, 25, 75);
    doc.text(`Email: ${saleData.customer_email || '-'}`, 25, 80);
  ...
  // Generate receipt as PDF buffer for backend use
  async generateReceiptBuffer(saleData) {
    const doc = this.generateReceipt(saleData);
    return Buffer.from(doc.output('arraybuffer'));
  }

  // Generate invoice as PDF buffer for backend use
  async generateInvoiceBuffer(saleData) {
    const doc = this.generateInvoice(saleData);
    return Buffer.from(doc.output('arraybuffer'));
  }

  // Save receipt to file system (backend only)
  async saveReceiptToFile(saleData, filePath) {
    const doc = this.generateReceipt(saleData);
  ...
  // Save invoice to file system (backend only)
  async saveInvoiceToFile(saleData, filePath) {
    const doc = this.generateInvoice(saleData);
    const fs = require('fs').promises;
    const buffer = Buffer.from(doc.output('arraybuffer'));
    await fs.writeFile(filePath, buffer);
